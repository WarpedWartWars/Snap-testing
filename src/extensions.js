...
var SnapExtensions = {
    ...
    urls: [
        'libraries/',
        'https://warpedwartwars.github.io/Snap-testing/'
    ]
};
...
SnapExtensions.primitives.set(
    ...
    function (thing, slot, target) {
        ...
        if (!(thing.expression instanceof BlockMorph ||
              thing.expression instanceof ReporterBlockMorph ||
              typeof thing === "string" ||
              typeof thing === "number" ||
              typeof thing === "boolean")) {return target; }

        var type = typeof thing,
            slotNum = 1;

        var outExpr = target.expression; //.fullCopy();
        var outCtx = new Context(null, outExpr);
        ...
        for (i in outExpr.children) {
            var child = outExpr.children[i];
            ...
            if (type === "boolean") {
                ...
                if (slotNum === slot) {
                    ...
                    if (child instanceof CSlotMorph) {
                        var input = child;
                        //clear children
                        while (input.children[0] !== undefined) {
...
SnapExtensions.primitives.set(
    ...
    function (script, child) {
        ...
        var tailBlock = script.expression.fullCopy(),
            outCtx = new Context(null, tailBlock),
            lastChild = tailBlock.nextBlock(),
            v;
...
SnapExtensions.primitives.set(
    ...
    function (val, expr) {
        ...
        if (type == "boolean") {
            ...
            var slotMorph = new BooleanSlotMorph();
        ...
        var outCtx = new Context(null, expr.expression),
            i;

        for (i in outCtx.expression.children) {
            var input = outCtx.expression.children[i];
    ...
    function (obj) {
        if (typeof obj === "object") {
            if (obj instanceof Context) {
                ...
                if (obj.expression) {
                    ctx.expression = obj.expression.fullCopy();
                }
                return ctx;
            }
    ...
    function (block) {
        block = (block.expression ?
                (block.expression instanceof Array ?
                    ...
                    block.expression) :
                block).fullCopy();
        var situation = block.situation() || {
            origin: world.children[0].currentSprite.scripts
        };
